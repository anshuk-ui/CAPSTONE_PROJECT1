...
Class Action {
	...
	function save_folder(){
		...
		if(empty($id)){
			...
			$check = $this->db->query("SELECT * FROM folders where user_id ='".$_SESSION['id']."' and name  ='".$name."'")->num_rows;
			if($check > 0){
			...
			$check = $this->db->query("SELECT * FROM folders where user_id ='".$_SESSION['id']."' and name  ='".$name."' and id !=".$id)->num_rows;
			if($check > 0){
				return json_encode(array('status'=>2,'msg'=> 'Folder name already exist'));
			}else{
